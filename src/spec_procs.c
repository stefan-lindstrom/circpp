...
SPECIAL(mayor)
{
  ...
  case 'W':
    ...
    act("$n awakens and groans loudly.", FALSE, ch, 0, 0, CommTarget::TO_ROOM);
    break;

  case 'S':
    GET_POS(ch) = POS_SLEEPING;
    act("$n lies down and instantly falls asleep.", FALSE, ch, 0, 0, CommTarget::TO_ROOM);
    break;

  case 'a':
    act("$n says 'Hello Honey!'", FALSE, ch, 0, 0, CommTarget::TO_ROOM);
    act("$n smirks.", FALSE, ch, 0, 0, CommTarget::TO_ROOM);
    break;
  ...
  case 'd':
    act("$n says 'Good day, citizens!'", FALSE, ch, 0, 0, CommTarget::TO_ROOM);
    break;

  case 'e':
    act("$n says 'I hereby declare the bazaar open!'", FALSE, ch, 0, 0, CommTarget::TO_ROOM);
    break;

  case 'E':
    act("$n says 'I hereby declare Midgaard closed!'", FALSE, ch, 0, 0, CommTarget::TO_ROOM);
    break;
...
void npc_steal(struct char_data *ch, struct char_data *victim)
{
  ...
  if (AWAKE(victim) && (rand_number(0, GET_LEVEL(ch)) == 0)) {
    act("You discover that $n has $s hands in your wallet.", FALSE, ch, 0, victim, CommTarget::TO_VICT);
    act("$n tries to steal gold from $N.", TRUE, ch, 0, victim, CommTarget::TO_NOTVICT);
  } else {
...
SPECIAL(snake)
{
  ...
  act("$n bites $N!", 1, ch, 0, FIGHTING(ch), CommTarget::TO_NOTVICT);
  act("$n bites you!", 1, ch, 0, FIGHTING(ch), CommTarget::TO_VICT);
...
SPECIAL(guild_guard)
{
  ...
  for (i = 0; guild_info[i].guild_room != NOWHERE; i++) {
    ...
    act(buf2, FALSE, ch, 0, 0, CommTarget::TO_ROOM);
    return (TRUE);
  }
...
SPECIAL(bank)
{
  ...
  if (CMD_IS("balance")) {
    ...
    act("$n makes a bank transaction.", TRUE, ch, 0, FALSE, CommTarget::TO_ROOM);
    return (TRUE);
