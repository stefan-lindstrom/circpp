...
void game_loop(socket_t mother_desc)
{
  ...
  while (!circle_shutdown) {
	...
	if (STATE(d) == CON_PLAYING && GET_WAS_IN(d->character) != NOWHERE) {
	  ...
	  act("$n has returned.", TRUE, d->character, 0, 0, CommTarget::TO_ROOM);
	}
...
void close_socket(struct descriptor_data *d)
{
  ...
  if (d->character) {
    ...
    if (STATE(d) == CON_PLAYING || STATE(d) == CON_DISCONNECT) {
      ...
      act("$n has lost $s link.", TRUE, link_challenged, 0, 0, CommTarget::TO_ROOM);
	 ...
	 struct obj_data *obj, const void *vict_obj, CommTarget type)
{
  ...
  /*
   * Warning: the following CommTarget::TO_SLEEP code is a hack.
   * 
   * I wanted to be able to tell act to deliver a message regardless of sleep
   * without adding an additional argument.  CommTarget::TO_SLEEP is 128 (a single bit
   * high up).  It's ONLY legal to combine CommTarget::TO_SLEEP with one other CommTarget::TO_x
   * command.  It's not legal to combine CommTarget::TO_x's with each other otherwise.
   * CommTarget::TO_SLEEP only works because its value "happens to be" a single bit;
   * do not change it to something else.  In short, it is a hack.
   */

  /* check if CommTarget::TO_SLEEP is there, and remove it if it is. */
  if (CommTarget::TO_SLEEP == (to_sleeping = (type & CommTarget::TO_SLEEP)))
    type &= ~CommTarget::TO_SLEEP;

  if (type == CommTarget::TO_CHAR) {
    if (ch && SENDOK(ch))
