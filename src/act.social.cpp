...
ACMD(do_action)
{
  ...
  if (!*buf) {
    send_to_char(ch, "%s\r\n", action->char_no_arg);
    act(action->others_no_arg, action->hide, ch, 0, 0, CommTarget::TO_ROOM);
    return;
  }
  if (!(vict = get_char_vis(ch, buf, NULL, FIND_CHAR_ROOM)))
    ...
    send_to_char(ch, "%s\r\n", action->char_auto);
    act(action->others_auto, action->hide, ch, 0, 0, CommTarget::TO_ROOM);
  } else {
    if (GET_POS(vict) < action->min_victim_position)
      act("$N is not in a proper position for that.", FALSE, ch, 0, vict, CommTarget::TO_CHAR | CommTarget::TO_SLEEP);
    else {
      act(action->char_found, 0, ch, 0, vict, CommTarget::TO_CHAR | CommTarget::TO_SLEEP);
      act(action->others_found, action->hide, ch, 0, vict, CommTarget::TO_NOTVICT);
      act(action->vict_found, action->hide, ch, 0, vict, CommTarget::TO_VICT);
    }
  }
}
...
void boot_social_messages(void)
{
  ...
  soc_mess_list = new social_messg[list_top + 1];
