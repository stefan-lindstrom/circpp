		      ...
		      int extract, int verbose)
{
  ...
  if (verbose) {
    ...
    act("A puff of smoke rises from $n's pack.", TRUE, ch, NULL, NULL, CommTarget::TO_ROOM);
  }
  return (TRUE);
}
		     ...
		     int spellnum, int savetype)
{
  ...
  case SPELL_DISPEL_EVIL:
    ...
    if (IS_EVIL(ch)) {
      ...
      act("The gods protect $N.", FALSE, ch, 0, victim, CommTarget::TO_CHAR);
      return (0);
    }
    break;
		      ...
		      int spellnum, int savetype)
{
  ...
  case SPELL_SLEEP:
    ...
    if (GET_POS(victim) > POS_SLEEPING) {
      ...
      act("$n goes to sleep.", TRUE, victim, 0, 0, CommTarget::TO_ROOM);
  ...
  if (to_room != NULL)
    act(to_room, TRUE, victim, 0, ch, CommTarget::TO_ROOM);
}
...
void mag_summons(int level, struct char_data *ch, struct obj_data *obj, int spellnum, int savetype)
{
  ...
  case SPELL_ANIMATE_DEAD:
    if (obj == NULL || !IS_CORPSE(obj)) {
      act(mag_summon_fail_msgs[7], FALSE, ch, 0, 0, CommTarget::TO_CHAR);
      return;
    }
    handle_corpse = TRUE;
    msg = 11;
  ...
  for (i = 0; i < num; i++) {
    ...
    act(mag_summon_msgs[msg], FALSE, ch, 0, mob, CommTarget::TO_ROOM);
    add_follower(mob, ch);
  }
  if (handle_corpse) {
...
void mag_alter_objs(int level, struct char_data *ch, struct obj_data *obj, int spellnum, int savetype)
{
  ...
  if (to_room != NULL)
    act(to_room, TRUE, ch, obj, 0, CommTarget::TO_ROOM);
  else if (to_char != NULL)
    act(to_char, TRUE, ch, obj, 0, CommTarget::TO_ROOM);
...
void mag_creations(int level, struct char_data *ch, int spellnum)
{
  ...
  obj_to_char(tobj, ch);
  act("$n creates $p.", FALSE, ch, tobj, 0, CommTarget::TO_ROOM);
  act("You create $p.", FALSE, ch, tobj, 0, CommTarget::TO_CHAR);
}
