  ...
  if (IS_NPC(ch))
	...
	for (d = descriptor_list; d; d = d->next)
	  if (STATE(d) == CON_PLAYING && d->character)
	    act(buf, FALSE, ch, 0, d->character, CommTarget::TO_VICT);
      } else
...
ACMD(do_gen_comm)
{
  ...
  for (i = descriptor_list; i; i = i->next) {
	...
	send_to_char(i->character, "%s", color_on);
      act(buf1, FALSE, ch, 0, i->character, CommTarget::TO_VICT | CommTarget::TO_SLEEP);
      if (COLOR_LEV(i->character) >= C_NRM)
...
ACMD(do_qcomm)
{
  ...
  if (!*argument)
    ...
    if (PRF_FLAGGED(ch, PRF_NOREPEAT))
      ...
      act(argument, FALSE, ch, 0, argument, CommTarget::TO_CHAR);
	...
	act(buf, 0, ch, 0, i->character, CommTarget::TO_VICT | CommTarget::TO_SLEEP);
  }
}
