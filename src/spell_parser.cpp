	     ...
	     struct obj_data *ovict, int spellnum, int level, int casttype)
{
  ...
  if (ROOM_FLAGGED(IN_ROOM(caster), ROOM_NOMAGIC)) {
    ...
    act("$n's magic fizzles out and dies.", FALSE, caster, 0, 0, CommTarget::TO_ROOM);
    return (0);
  }
  if (ROOM_FLAGGED(IN_ROOM(caster), ROOM_PEACEFUL) &&
    ...
    act("White light from no particular source suddenly fills the room, then vanishes.", FALSE, caster, 0, 0, CommTarget::TO_ROOM);
    return (0);
  }
		          ...
		          char *argument)
{
  ...
  case ITEM_STAFF:
    ...
    if (GET_OBJ_VAL(obj, 2) <= 0) {
      ...
      act("Nothing seems to happen.", FALSE, ch, obj, 0, CommTarget::TO_ROOM);
    } else {
      GET_OBJ_VAL(obj, 2)--;
  ...
  case ITEM_WAND:
	...
	act("$n points $p at $P.", TRUE, ch, obj, tobj, CommTarget::TO_ROOM);
      ...
      act("$n points $p outward.", TRUE, ch, obj, NULL, CommTarget::TO_ROOM);
    } else {
      act("At what should $p be pointed?", FALSE, ch, obj, NULL, CommTarget::TO_CHAR);
      return;
    }
