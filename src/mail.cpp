...
char *read_delete(long recipient)
{
  ...
  if (!(position_pointer = mail_pointer->list_start)) {
    basic_mud_log("SYSERR: Mail system -- non-fatal error #8. (invalid position pointer %p)", reinterpret_cast<void *>(position_pointer));
    return (NULL);
  }
...
void postmaster_send_mail(struct char_data *ch, struct char_data *mailman, int cmd, char *arg)
{
  ...
  if (GET_LEVEL(ch) < MIN_MAIL_LEVEL) {
    ...
    act(buf, FALSE, mailman, 0, ch, CommTarget::TO_VICT);
    return;
  }
  ...
  if (!*buf) {			/* you'll get no argument from me! */
	...
	FALSE, mailman, 0, ch, CommTarget::TO_VICT);
    return;
  }
  ...
  act("$n starts to write some mail.", TRUE, ch, 0, 0, CommTarget::TO_ROOM);
...
void postmaster_check_mail(struct char_data *ch, struct char_data *mailman, int cmd, char *arg)
{
  ...
  if (has_mail(GET_IDNUM(ch)))
    act("$n tells you, 'You have mail waiting.'", FALSE, mailman, 0, ch, CommTarget::TO_VICT);
  else
    act("$n tells you, 'Sorry, you don't have any mail waiting.'", FALSE, mailman, 0, ch, CommTarget::TO_VICT);
}
...
void postmaster_receive_mail(struct char_data *ch, struct char_data *mailman, int cmd, char *arg)
{
  ...
  while (has_mail(GET_IDNUM(ch))) {
    ...
    act("$n gives you a piece of mail.", FALSE, mailman, 0, ch, CommTarget::TO_VICT);
    act("$N gives $n a piece of mail.", FALSE, ch, 0, mailman, CommTarget::TO_ROOM);
  }
}
