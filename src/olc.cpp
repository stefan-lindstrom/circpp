...
/* local functions */
void olc_interpreter(void *targ, OlcMode mode, char *arg);
void olc_set_show(struct char_data *ch, OlcMode olc_mode, char *arg);
...
ACMD(do_olc)
{
  ...
  if ((mode_search = search_block(mode_arg, olc_modes, FALSE)) < 0) {
    send_to_char(ch, "Invalid mode '%s'.\r\n%s", mode_arg, OLC_USAGE);
    return;
  }
  olc_mode = static_cast<OlcMode>(mode_search);

  switch (olc_mode) {
  case OlcMode::OLC_SET:
  case OlcMode::OLC_SHOW:
    olc_set_show(ch, olc_mode, argument);
    return;
  case OlcMode::OLC_REPEAT:
    if ((OlcMode::OLC_SET != (olc_mode = GET_LAST_OLC_MODE(ch))) ||
...
void olc_interpreter(void *targ, OlcMode mode, char *arg)
{
  ...
  switch (static_cast<OlcCmd>(command)) {
  case OlcCmd::OLC_COPY:
    switch (mode) {
    case OlcMode::OLC_ROOM:
      break;
    case OlcMode::OLC_MOB:
