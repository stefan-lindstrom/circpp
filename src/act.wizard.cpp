...
ACMD(do_echo)
{
  ...
  if (!*argument)
    ...
    if (PRF_FLAGGED(ch, PRF_NOREPEAT))
      ...
      act(buf, FALSE, ch, 0, 0, CommTarget::TO_CHAR);
  }
}
...
ACMD(do_trans)
{
  ...
  if (!*buf)
	...
	return;
      ...
      act("$n disappears in a mushroom cloud.", FALSE, victim, 0, 0, CommTarget::TO_ROOM);
      char_from_room(victim);
      char_to_room(victim, IN_ROOM(ch));
      act("$n arrives from a puff of smoke.", FALSE, victim, 0, 0, CommTarget::TO_ROOM);
      act("$n has transferred you!", FALSE, ch, 0, victim, CommTarget::TO_VICT);
...
{ 
  ...
  if (!*buf)
    ...
    send_to_char(ch, "%s", OK);
    act("$n disappears in a puff of smoke.", FALSE, victim, 0, 0, CommTarget::TO_ROOM);
    char_from_room(victim);
    char_to_room(victim, target);
    act("$n arrives from a puff of smoke.", FALSE, victim, 0, 0, CommTarget::TO_ROOM);
    act("$n has teleported you!", FALSE, ch, 0, (char *) victim, CommTarget::TO_VICT);
    look_at_room(victim, 0);
  }
}
...
ACMD(do_load)
{
  ...
  if (is_abbrev(buf, "mob")) {
	...
	0, 0, CommTarget::TO_ROOM);
    act("$n has created $N!", FALSE, ch, 0, mob, CommTarget::TO_ROOM);
    act("You create $N.", FALSE, ch, 0, mob, CommTarget::TO_CHAR);
    ...
    act("$n makes a strange magical gesture.", TRUE, ch, 0, 0, CommTarget::TO_ROOM);
    act("$n has created $p!", FALSE, ch, obj, 0, CommTarget::TO_ROOM);
    act("You create $p.", FALSE, ch, obj, 0, CommTarget::TO_CHAR);
  } else
...
ACMD(do_advance)
{
  ...
  if (newlevel < GET_LEVEL(victim)) {
	...
	"You feel slightly different.", FALSE, ch, 0, victim, CommTarget::TO_VICT);
  }
...
ACMD(do_restore)
{
  ...
  if (!*buf)
    ...
    affect_total(vict);
    send_to_char(ch, "%s", OK);
    act("You have been fully healed by $N!", FALSE, vict, 0, ch, CommTarget::TO_CHAR);
  }
}
...
ACMD(do_wizutil)
{
  ...
  if (!*arg)
	...
	return;
      ...
      act("A sudden cold wind conjured from nowhere freezes $n!", FALSE, vict, 0, 0, CommTarget::TO_ROOM);
