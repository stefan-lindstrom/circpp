			      ...
			      long cost)
{
  ...
  act(buf, FALSE, recep, 0, ch, CommTarget::TO_VICT);
}
		         ...
		         int display, int factor)
{
  ...
  if (!numitems) {
	...
	FALSE, recep, 0, ch, CommTarget::TO_VICT);
    return (0);
  }
  ...
  if (display) {
    ...
    if (totalcost > GET_GOLD(ch) + GET_BANK_GOLD(ch)) {
      act("$n tells you, '...which I see you can't afford.'", FALSE, recep, 0, ch, CommTarget::TO_VICT);
      return (0);
...
int gen_receptionist(struct char_data *ch, struct char_data *recep, int cmd, char *arg, int mode)
{
  ...
  if (!CAN_SEE(recep, ch)) {
    act("$n says, 'I don't deal with people I can't see!'", FALSE, recep, 0, 0, CommTarget::TO_ROOM);
    return (TRUE);
  }
  ...
  if (CMD_IS("rent")) {
    ...
    if (mode == RENT_FACTOR) {
      act("$n stores your belongings and helps you into your private chamber.", FALSE, recep, 0, ch, CommTarget::TO_VICT);
    ...
    act("$n helps $N into $S private chamber.", FALSE, recep, 0, ch, CommTarget::TO_NOTVICT);

    GET_LOADROOM(ch) = GET_ROOM_VNUM(IN_ROOM(ch));
    extract_char(ch);	/* It saves. */
  } else {
    Crash_offer_rent(ch, recep, TRUE, mode);
    act("$N gives $n an offer.", FALSE, ch, 0, recep, CommTarget::TO_ROOM);
  }
  return (TRUE);
}
